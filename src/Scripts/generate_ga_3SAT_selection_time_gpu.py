...
for job_id, (popsize, selection) in combinations:
    with open(f'{args.output}/_job.{ctime}.{job_id}.sh', "w") as f:
        print(f"#!/bin/bash", file=f)
        print(f"#PBS -N GPU_GA_SELECT", file=f)
        ...
        print(file=f)
        print(f"python ./time_ga_3SAT_selection.py", end=" ", file=f)
        ...
        print(f"--selection {selection}", end=" ", file=f)
