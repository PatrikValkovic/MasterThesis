...
ctime = int(time.time())
machines = ['pcr16', 'pcr14','pcr13', 'pcr12', 'pcr11', 'pcr10','pcr9','pcr7']
hyper = {
    'circle': 0.13,
    'nearest': 0.13,
    'grid_linear': 0.005,
...
for job_id, (neigh_type, fn) in combinations:
    with open(f'{args.output}/_job.{ctime}.{job_id}.sh', "w") as f:
        ...
        print(f"#PBS -N PSO06_CPU_neigh", file=f)
        print(f"#PBS -l select=1:ncpus={args.take_cores_per_job}:mem=32gb:scratch_local=50gb:vnode={machines[job_id // jobs_per_machine]}", file=f)
        ...
        print(f"python time_pso2006_neighborhood.py --neigh_size {hyper[neigh_type]} --neigh_type {neigh_type} --function {fn} --dim 128 --popsize \"1225,36,121,225,529,2500,4900,10000,16900,22500\" --device \"cpu\" --repeat 100 --cpu_count {args.cores_per_job}", file=f)
        print(file=f)
