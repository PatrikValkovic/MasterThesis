...
\end{itemize}
These models are historically more suited for multi--processor systems rather than for \gpuns, as it would be hard to scale them up to thousands of cores, that are currently available in modern \gpuns.
...
\end{itemize}
The gene--level granularity is ideal for the \gpu computing, as it expose enough parallelism. Earlier attempts with chromosome--level or more coarse level granularity could not fully utilize the \gpuns.