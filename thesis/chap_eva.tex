...
$$
where $F$ is user--defined positive parameter controlling the strength of the direction. The vector $\mathbf{p_2} - \mathbf{p_3}$ is the direction from parent $\mathbf{p_3}$ to parent $\mathbf{p_2}$ and reflects the correlation within the population. One example is in the figure \ref{fig:differentialevolution}. Notice how newly created individual is much closer to the optimum compared to it's parents.

There is possibility to use more parents and combine them. Moreover, there are possibilities not to sample parents uniformly, but substitute the best individual in the population as either $\mathbf{p_1}$ or $\mathbf{p_2}$. Alternatively, it is possible to use same individual as the source of the direction as well as the starting position, that is $\mathbf{p_1} = \mathbf{p_3}$. Formula for crossover with five parents is
$$
...
\acrfull{acc:pso}\index{particle swarm optimization} is another population--based stochastic optimization technique inspired by a flocking of birds and schooling of fish in Nature. It was introduced by \citet*{PSOOriginal}. The \acrshort{acc:pso} is somehow similar to the \acrshort{acc:es} in a sense that the individuals are vector of real numbers. In the \acrshort{acc:pso} terminology, the individuals are called \emph{particles}.

Thorough the time, the \acrshort{acc:pso} went through number of improvements. Two major versions, namely \acrfull{acc:spso2006} and \acrfull{acc:spso2011}, were released as a baseline for further improvements. I will focus on these versions as specified by \citet{SPSO}.

Expect the position, each particle $\mathbf{p_i}$ has velocity vector $\mathbf{v_i}$, which defines position of the particle in the following step. Unlike \acrshort{acc:es}, modification to the particles are made directly, but through manipulating with the velocity vector. These two components are cause why the \acrshort{acc:pso} get its name.

Additionally to velocity, each particle $\mathbf{p_i}$ remember it's own best position, usually called \emph{local best}\index{particle swarm optimization!local best} and denoted by $\mathbf{L_i}$. This variable is updated after every step, if the fitness value of particle's current position is better then the fitness value of $\mathbf{L_i}$.

Similarly, each particle remember best position it received from the swarm, usually called \emph{global best}\index{particle swarm optimization!global best} and denoted by $\mathbf{G_i}$. After each step, each particle informs surrounding particles about it's position and fitness value. Each particle may inform different set of particles. Which particles to inform is defined by particle's \emph{neighborhood} and I will focus on in a bit later. Each particle then accumulate information from it's surrounding and may update it's global best position $\mathbf{G_i}$.
...
Neighborhood\index{particle swarm optimization!neighborhood} or also called topology specifies connection between particles and how the particles communicate their best positions in the swarm. It directly controls information flow in the swarm and therefore speed of convergence. For big neighborhood, the particles soon receive best position in the swarm and will attempt to reach it without much exploring. If is the neighborhood too short, particles may focus too much on the exploration as the global information would be missing.
...
Notice that all these topologies are static and does not change during run of the algorithm. Dynamic topologies were proposed, such as random topology and closest neighbors topology. In the random topology, each particle inform $k$ random particles from the swarm. The $k$ is usually kept small, for example $k=3$ \citep{SPSObenchmark}. Note that different neighborhood is generated after each step. In the closest neighbors topology each particle informs $k$ closest particles -- except that this topology is very computation intensive, it usually lead to premature convergence, because particles begin to create clusters and keep informing themselves only within this cluster.
...
$$ 
where $\omega$ is inertia weight, $c_l$ and $c_g$ are cognitive respectively social acceleration coefficients, and $U_d(0,1)$ is independent and uniformly distributed random vector in the range $\left[ 0,1 \right]$ with dimension $d$. The $\cdot$ is element--wise multiplication. The inertia weight prevent from explosion of the velocity. Constants $c_l$ and $c_g$ controls, how much the local best respectively the global best attracts the particle, and are specified by the user. The particle position it then updated simply by adding applying velocity to the particle.
$$
\mathbf{p_i} = \mathbf{p_i} + \mathbf{v_i}
$$

Except inertia weight, one may limit the position of the particle to fit into interval of interest, respectively limit velocity. We may limit the velocity using its magnitude (then no particle can move faster than specified threshold), or per coordinate, similarly to the position \citep{PSOvelocitylimit}.
...
Because the velocity update in \acrshort{acc:spso2006} is done dimension by dimension, the performance of the algorithm is rotation sensitive and depends is coordinate system dependent. This phenomena is visible in figure \ref{fig:samplingspso2006}. The sampled point for local respectively global best position is sampled from the blue respectively orange region. As these regions are aligned with axes, the resulting sampling is aligned with axes as well. It has been shown \citep{psobiasinzero} that it is easier for \acrshort{acc:spso2006} to find optimum, if it lies in the center of coordinate systems or on the axis.
...
$$
Velocity update then simply add direction to this point to the previous velocity.
$$
\mathbf{v_i} = \omega\mathbf{v_i}+\mathbf{u_i}-\mathbf{p_i}
$$
The sample space for two dimensions is depict in figure \ref{fig:samplingspso2011}.
