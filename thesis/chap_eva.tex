...
This exact ideas inspired scientists and formed the field of \acrfull{acc:ea}\index{evolutionary algorithm}\footnote{I will refer to all the population-based algorithms in this work as \acrlong*{acc:ea}. These include, among others, \acrlong*{acc:ga}, \acrlong*{acc:es}, and \acrlong*{acc:pso}.}.
...
\defterm{Problem} is the function we want to solve. It can be both a satisfactory or optimization problem.

\defterm{Genotype}\index{genotype} is encoding of the solution. It is equivalent of the \acrshort{acc:dna} -- it compresses all the data needed to produce solution.
...
\defterm{Objective function} is the function the algorithm optimizes. For the previous example of real function optimization, the function itself can be the objective function. I will denote objective function $\objectivefn$.
...
\defterm{Evaluation} is the process of obtaining fitness value for a given genotype.

\defterm{Individual} is a genotype with its fitness value. I will denote individual by $\individual$ and his fitness $\fitnessfn(\individual)$.
...
\defterm{Evolution operators} are individual steps performing each iteration.
...
One way to eliminate this problem is to rescale fitness to have more desired properties\index{fitness scaling}. One common example is linear scaling into interval $\left[ l,u \right]$ so that individual with $\min\fitnessfn(x_i)$, respectively $\max\fitnessfn(x_i)$, has new fitness equal to $l$, respecitvely $u$; and rest of them is scaled linearly between them. 
$$
...
Let's take for example two functions:
\begin{align*}
    f_{sep}(x,y)&=x^2+3y^2 \\
...
One of the simplest real--coded \acrshort{acc:ea} crossover is \emph{arithmetic crossover}\index{crossover real--coded!arithmetic}. For parents $\mathbf{p_1}$ and $\mathbf{p_2}$, the offspring $\mathbf{o}$ is created using following formula.
$$
...
Arithmetic crossover is just a linear combination of parents, it does not satisfy condition on page \pageref{enum:espopulationvariance} to \enquote{\snipescondition}. Authors \citet{BlendCrossoverOriginal} address this issue in \emph{blend crossover}\index{crossover real--coded!blend} operator. For parents $\mathbf{x}$ and $\mathbf{y}$, the genes of offspring $\mathbf{o}$ are sampled randomly from uniform distribution specified by the parents.
$$
o_i = U\left( 
...
The equivalent of bit--flip mutation in \acrshort{acc:ga} is \emph{uniform mutation}\index{mutation real--coded!uniform}. With small probability $p_m$ it replaces gene by a random value sampled from $U(l,u)$, where interval $\left[l,u\right]$ is the scalar's domain. This operator basically tries random numbers and can rarely lead to a good solution. Moreover, it does not exploit the distribution of the population.
